...
uploadForm.addEventListener("submit", async function (e) {
  ...
  try {
    ...
    if (result.success) {
      if (result.job_id && result.mode === "async") {
        // معالجة غير متزامنة مع Celery
        console.log("🔄 استخدام Celery للمعالجة غير المتزامنة");
        trackJobProgress(result.job_id, result.output_filename);
      } else if (result.mode === "direct") {
        // معالجة مباشرة مكتملة
        console.log("✅ معالجة مباشرة مكتملة");
        showResult(result);
      } else {
        // حالة غير معروفة
        showError("نمط معالجة غير معروف", result);
      }
    } else {
    ...
    showError(
      "حدث خطأ في الاتصال بالخادم - تحقق من Console للتفاصيل",
      errorDetails
    );
  }
});
...
async function downloadErrorLog() {
  try {
    ...
    a.href = url;
    a.download = `error_log_${new Date()
      .toISOString()
      .replace(/[:.]/g, "-")}.json`;
